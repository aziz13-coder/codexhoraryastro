...
def extract_testimonies(chart: HoraryChart, contract: Dict[str, Planet]) -> List[Any]:
    ...
    if not relevant_houses:
        ...
        if chart_category == "career":
            ...
            # Generic fallback - use traditional testimonial houses
            relevant_houses.update({2, 5, 7, 8})
...
class EnhancedTraditionalHoraryJudgmentEngine:
        ...
        try:
            ...
            if judgment.get("hybrid_confidence_needed"):
                ...
                hybrid_result = self._calculate_hybrid_confidence(
                    ...
                    evaluation["score"], 
                    blockers, 
                    70  # Base confidence in the determined verdict
                )
    ...
    def _apply_enhanced_judgment(self, chart: HoraryChart, question_analysis: Dict,
        ...
        if perfection["perfects"]:
            ...
            # For translation/collection of light, ensure minimum confidence despite penalties
            if perfection.get("type") in ["translation", "collection"]:
                # Traditional doctrine: perfection exists, so minimum confidence should be meaningful
                perfection_confidence = perfection.get("confidence", 50)
                confidence = max(perfection_confidence, min(confidence, perfection_confidence))
            else:
        ...
        if not direct_aspect_found:
            ...
            if translation and translation.get("found", False):
                ...
                return {
                    "perfects": True,
                    "type": "translation",
                    "favorable": translation["favorable"],  # Now always True for translation
                    "confidence": max(translation.get("confidence", 50), 40),  # Ensure minimum confidence
                    "reason": enhanced_reason,
                    "translator": translation["translator"],
                    "challenge_reasons": challenge_reasons,  # New field for challenges
                    "negative_reasons": translation.get("negative_reasons", []),  # Backward compatibility
